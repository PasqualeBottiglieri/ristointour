...
export default function SocialLinksInput({ defaultValue }: SocialLinksInputProps) {
  const initialValues: Record<string, string> = {};
  if (defaultValue) {
    for (const link of defaultValue) {
      initialValues[link.platform] = link.url;
    }
  }
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {PLATFORMS.map((p) => (
          <div key={p.key} className="flex items-center gap-3">
            ...
            <span className="text-xs font-medium text-stone-600 w-20 shrink-0">
              {p.label}
            </span>
            <input
              type="url"
      ...
      <input type="hidden" name="socialLinks" value={jsonValue} />
    </div>
  );
}
